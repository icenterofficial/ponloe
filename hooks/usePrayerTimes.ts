...
export const usePrayerTimes = (
  ...
  useEffect(() => {
    let isMounted = true;
        const fetchTimes = async (retryCount = 0) => {
      setLoading(true);
      ...
      const baseUrl = '';
      const apiUrl = `${baseUrl}/api/aladhan/calendar?year=${year}&month=${month}&latitude=${latitude}&longitude=${longitude}&method=${calculationMethod}&school=${juristicMethod}&adjustment=${hijriAdjustment}`;
      ...
      try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
          ...
          if ((text.includes('Starting Server...') || text.includes('<html') || text.includes('502 Bad Gateway') || text.includes('503 Service Temporarily Unavailable')) && retryCount < 5) {
            // Server is still starting or proxy error, retry after a delay
            setTimeout(() => {
              if (isMounted) fetchTimes(retryCount + 1);
            }, 2000);
            return;
          }
  ...
  }, [provinceKey, districtIndex, customLat, customLon, calculationMethod, juristicMethod]);
